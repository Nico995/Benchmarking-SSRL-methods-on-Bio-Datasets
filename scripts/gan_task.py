...
def main():
    ...
    if args.optimizer == "SGD":
        gen_optimizer = SGD(model.backbone.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        disc_optimizer = SGD(model.discriminator.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    elif args.optimizer == "Adam":
        gen_optimizer = Adam(model.parameters(), weight_decay=args.weight_decay)
        disc_optimizer = Adam(model.parameters(), weight_decay=args.weight_decay)
    else:
        NotImplementedError()
    optimizer = [gen_optimizer, disc_optimizer]

    # Load learning rate scheduler
    gen_lr_scheduler = MultiStepLR(gen_optimizer, [80, 100], 0.1)
    disc_lr_scheduler = MultiStepLR(disc_optimizer, [80, 100], 0.1)
    lr_scheduler = [gen_lr_scheduler, disc_lr_scheduler]
